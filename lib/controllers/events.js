...
var filter = function(list, filterKey, filterValue) {
  var i, j, k,
    evt,
    results = [],
    vals = filterValue.split(',');

  for (i=0; i < list.length; i++) {
    evt = list[i];
    
    if (Array.isArray(evt[filterKey])) {
      for (k=0; k<evt[filterKey].length; k++) {
        for (j=0; j<vals.length; j++) {
          if (evt[filterKey][k] === vals[j]) {
            results.push(evt);
          }
        }
      }
...
exports.getCategories = function(callback) {
  eventsDb.view('category/categories', { group: true },
    function(err, results) {
      var categories = [], i;
...
exports.addCategories = function(req, res, next) {
  exports.getCategories(function(err, categories) {
...
exports.resetDb = function() {
  eventsDb.destroy(function() {
    eventsDb.create(function() {
      ...
      eventsDb.save('_design/category', {
        categories: {
          map: function(doc) {
